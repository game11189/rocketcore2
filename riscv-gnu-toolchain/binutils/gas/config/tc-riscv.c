...
riscv_set_arch (const char *arg)
{
  ...
  char *p = uppercase;
  const char *all_subsets = "IMAFDCV"; // need to be in the correct order GAME
...
reg_lookup_internal (const char *s, enum reg_class class)
{
  ...
  //printf("DECODE_REG_NUM = %d\n", DECODE_REG_NUM (r));
  return DECODE_REG_NUM (r);
}

//if (reg_lookup (&s, RCLASS_VECT, &regno))
...
#define USE_BITS(mask,shift)	(used_bits |= ((insn_t)(mask) << (shift)))
  while (*p)
	...
	switch (c = *p++)
	  {
	  case 'a': USE_BITS (OP_MASK_RD, OP_SH_RD); break;
	  case 'b': USE_BITS (OP_MASK_RS1, OP_SH_RS1); break;
	  case 'c': USE_BITS (OP_MASK_RS2, OP_SH_RS2); break;
	  case 's': USE_BITS (OP_MASK_RS1, OP_SH_RS1); break;
	  case 't': USE_BITS (OP_MASK_RS2, OP_SH_RS2); break;
	  }
	break;
...
md_begin (void)
{
  ...
  hash_reg_names (RCLASS_VECT, riscv_vect_names_abi, NVPR);
...
rvc_lui:
		...
		switch (*++args)
		{
		printf("Va, Vb, Vc lookup\n");
		case 'a':		/* destination register */
	    	case 'b':		/* source register */
	    	case 'c':		/* target register */
			if (reg_lookup (&s, RCLASS_GPR, &regno))
			{
				//printf("LOOKUPPPP\n");
		  		c = *args;
		  ...
		  /* Now that we have assembled one operand, we use the args
		     string to figure out where it goes in the instruction.  */
		  	switch (c)
		    	{
		    	case 'b':
				//printf("INSERT_OPERAND b\n");
		      		INSERT_OPERAND (RS1, *ip, regno);
		      		break;
		    	case 'a':
				//printf("INSERT_OPERAND a\n");
		     	 	INSERT_OPERAND (RD, *ip, regno);
		      		break;
		    	case 'c':
				//printf("INSERT_OPERAND c\n");
...
riscv_after_parse_args (void)
{
  ...
  if (riscv_subsets == NULL)
    riscv_set_arch ("RVIMAFDVXcustom");
  if (xlen == 0)
    {
...
tc_riscv_regname_to_dw2regnum (char *regname)
{
  ...
  if ((reg = reg_lookup_internal (regname, RCLASS_VECT)) >= 0)
    return reg + 64 ;
