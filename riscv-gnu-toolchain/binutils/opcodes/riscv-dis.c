...
static const char * const *riscv_vect_names;
/* Other options */
...
set_default_riscv_dis_options (void)
{
  ...
  riscv_vect_names = riscv_vect_names_abi;
  no_aliases = 0;
}
  ...
  for (; *d != '\0'; d++)
	...
	case 'V':
	  switch (*++d)
	    {
	    case 'a':
	      print (info->stream, "%s", riscv_vect_names[rd]);
	      break;
	    case 'b':
	      print (info->stream, "%s", riscv_vect_names[rs1]);
	      break;
	    case 'c':
	      print (info->stream, "%s", riscv_vect_names[EXTRACT_OPERAND (RS2, l)]);
	      break;
	    }
	  break;
	case 'C': /* RVC */
